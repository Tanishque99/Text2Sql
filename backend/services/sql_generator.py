...
class SQLGenerator:
    def __init__(
        ...
        if self.use_api:
            if self.custom_api_url:
                ...
                # Using router endpoint which is the current supported method
                self.api_url = (
    ...
    def generate_sql(
        ...
        try:
            ...
            if self.use_api:
                sql_query = self._generate_with_api(prompt, max_new_tokens, question)
            else:
    ...
    def _generate_with_api(self, prompt: str, max_new_tokens: int = 128, question: str = "") -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_msg = (
                    ...
                    f"The model may not be available on HuggingFace Inference API.\n"
